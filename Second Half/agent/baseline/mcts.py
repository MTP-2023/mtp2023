...
def mcts(root_state, max_iterations, exploration_constant, goalstate, width, height, max_steps, n_steps, player):
    ...
    for i in range(max_iterations):
        ...
        while len(node.children) == width:
            ...
            state = node.state
        child = createChild(node, width)
        result = simulate(copy.deepcopy(child.state), width, height, max_steps, node.depth + n_steps, goalstate)
...
def simulate(state, width, height, max_steps, n_steps, goalstate, current_player):
    ...
    while not reward(state, width, height, goalstate, n_steps, max_steps)[1] and n_steps < max_steps:
        current_player *= -1
