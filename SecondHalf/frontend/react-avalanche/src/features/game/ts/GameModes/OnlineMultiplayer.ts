...
export class OnlineMultiPlayer extends AbstractGameMode {
    ...
    public newGameEvent: EventEmitter;
        ...
        this.ws.onmessage = (event) => {
            ...
            switch(lobby.messageType){
                ...
                case "pls confirm":
                    ...
                    break;
                case "confirmed":
                    this.newGameEvent.emit("confirmed");
                    break;
            }
        };
    ...
    public async confirmNextMatch(){
        
        this.ws.send(JSON.stringify(new MessageAvalanche(MessageType.CONFIRMPLAYAGAIN, "")));
    }
    ...
    public createPlayerStatus(scene: Phaser.Scene, x: number, y: number, width: number, height: number, boardEnd: number, player1Text: string, player2Text: string, imgLabel: string, imgScale: number): void {
        ...
        const scoreText1 =  scene.add.text(0, 100, this.player1Wins.toString(), { fontSize: 36, fontFamily: "monospace", color: this.convertToCSS(this.player1Color), align: "center" }).setOrigin(0.5);
        ...
        const scoreText2 =  scene.add.text(0, 100, this.player2Wins.toString(), { fontSize: 36, fontFamily: "monospace", color: this.convertToCSS(this.player2Color), align: "center" }).setOrigin(0.5);
